...
if (isset($_GET['id_livre'])) {
    ...
    $sql = "SELECT * FROM dvd WHERE id = '$id_livre'";
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && empty($message_erreur)) {
    $sql_check_emprunt = "SELECT * FROM emprunt WHERE id_livre = '$id_livre' AND retour_effectif IS NULL AND type = 'dvd'";
    ...
    if (mysqli_num_rows($result_precommande) > 0) {
        ...
        VALUES ('$id_livre', '$id_abonne', '$email', '$date_emprunt', '$date_retour', 'dvd')
        ";
...
<body>
    ...
    <a href="dvd.php"><button>Retourner voir les DVD</button></a>
...
<?php else: ?>
    ...
    <form action="confirmation_emprunt_dvd.php?id_livre=<?php echo $livre['id']; ?>" method="post">
