...
<header>
    ...
    <?php endif; ?>
                ...
                <li><a href="titres_periodiques.php">Titres Périodiques</a></li>
            </ul>
    ...
    <main>
        ...
        if ($isConnected) {
            ...
            <p class="warning">Vous avez atteint la limite de 4 emprunts simultanés. Merci de rendre un livre avant d'en emprunter un nouveau.</p>
        <?php endif; ?>
        <?php } else { ?>
        ...
        <?php if ($isAbonne): ?>
            <?php if ($row['statut'] === 'disponible' && !$hasLateEmprunt && $nbEmpruntsActifs < 4): ?>
                <a class="voir-details" href="confirmation_emprunt_livre.php?id_livre=<?php echo $row['id']; ?>">Voir les détails</a>

                <?php elseif ($row['statut'] === 'emprunte'): ?>
    <?php if (in_array($row['id'], $livresEmpruntesParMoi)): ?>
