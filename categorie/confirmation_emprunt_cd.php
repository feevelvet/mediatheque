...
if (isset($_GET['id_livre'])) {
    ...
    $sql = "SELECT * FROM cd WHERE id = '$id_livre'";
    ...
    if (mysqli_num_rows($result) > 0) {
        ...
        if ($livre['statut'] == 'emprunte') {
            $disableButton = true;
            $buttonMessage = "Veuillez nous excuser ce livre semble déjà avoir été emprunté !";
        }
    } else {
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && empty($message_erreur)) {
    $sql_check_emprunt = "SELECT * FROM emprunt WHERE id_livre = '$id_livre' AND retour_effectif IS NULL AND type = 'cd'";
    ...
    if (mysqli_num_rows($result_precommande) > 0) {
        ...
        VALUES ('$id_livre', '$id_abonne', '$email', '$date_emprunt', '$date_retour', 'cd')
        ";
...
<body>
    ...
    <main>
        ...
        <div class="livre-details">
            ...
            $titleWithoutSpaces = strtolower(str_replace(' ', '_', $livre['titre']));
            ...
            if (file_exists('../assets/' . $imageName)): ?>
                <img src="../assets/<?php echo $imageName; ?>" alt="<?php echo htmlspecialchars($livre['titre']); ?>" width="150" height="225">
            <?php else: ?>
            ...
            <p><strong>Description:</strong> <?php echo nl2br(htmlspecialchars($livre['description'])); ?></p>
                ...
                <p><strong>🎬 Durée:</strong> <?php echo htmlspecialchars($livre['duree']); ?> minutes</p>
    ...
    <a href="cd.php"><button>Retourner voir les CD</button></a>


    

        <a href="livre.php"><button>Voir les livres</button></a>
        <a href="dvd.php"><button>Voir les CDs</button></a>
        <a href="titres_periodiques.php"><button>Retourner voir les titres_periodiques</button></a>

<?php else: ?>


    <form action="confirmation_emprunt_cd.php?id_livre=<?php echo $livre['id']; ?>" method="post">
